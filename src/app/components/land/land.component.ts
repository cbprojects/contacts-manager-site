...
export class LandComponent implements OnInit {
  ...
  nodoIndustriaSelect: TreeNode;
  contactoModel: ContactoModel;
  verMasIndustrias: boolean;
  industries: any[];
  listaRefPorcentajesUri: any[];
  displayContactoOK: boolean;
  ...
  seleccionarIndustria(industria) {
  ...
  obtenerDescripcionIndustria(idIndustria) {
    let descripcion = "";
    switch (idIndustria) {
      case 1:
  ...
  crearNodo(labelTxt, bgImgn, idRowNodo, mensaje: string) {
    return {
      label: labelTxt,
      type: 'industryNoImg',
      styleClass: 'p-person',
